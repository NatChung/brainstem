...
4. **codegraph 架構**(用 **CLI 不用 MCP**;MCP 綁呼叫端 repo 會污染使用者索引):
   - **規模閘(init 前)**:`git -C <TEMP> ls-files | wc -l` 估檔數;**> ~5000 → 不全倉 init**,改**對最相關子目錄 init 再 query**(`codegraph init <TEMP>/<subdir>` → `codegraph query -p <TEMP>/<subdir> …` / `codegraph files -p <TEMP>/<subdir>`)、或退回讀關鍵檔,回報註明「未全倉索引」。(`codegraph files`/`query` 讀 index,沒先 init 必失敗。)
