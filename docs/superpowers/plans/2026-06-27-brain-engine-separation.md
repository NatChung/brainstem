...
**Task order note:** `install.sh` copies `init.mjs` and `_brain-template/` into `ENGINE_HOME`, so those must exist first. Hence ordering: engine libs (T1–T3) → template (T4) → init (T5) → **then** dispatcher+install (T6) → skills (T7) → docs/cleanup (T8). Do not reorder T6 earlier.
...
```
(`fileURLToPath`/`dirname` were only used for `BRAIN` at line 10 — confirmed they appear nowhere else — so drop them. `join` stays; still used below.)
...
Run: `bash bin/test-doctor.sh`
Expected: `PASS` (all three cases). Note: when run via `BRAIN_DIR`, find-brain returns it directly; doctor's own checklist handles the `.brainroot`/lens checks exactly as before. `Bun.which("brainstem")` is null in the test → yellow `warn`, not red, so exit codes are unaffected.
...
```
(Note: `_templates/lens.md` is intentionally NOT moved — it's dropped from the engine entirely in Task 8.)
...
## Phase 3 — Global install & CLI

### Task 6: `bin/brainstem` dispatcher + copy-based `install.sh` + fix tests

> All files `install.sh` copies (`init.mjs`, `_brain-template/`, `lib/`, `check.mjs`, `doctor.mjs`, `VERSION`) now exist (Tasks 1–5), so its `cp -R` succeeds and `bin/test-install.sh` passes standalone.
...
Expected: not yet `PASS` (root still brain-shaped / cleanup pending). This is the gate that must go green after Steps 3–6.
...
git rm -f _templates/lens.md           # 被刻意丟棄的那一個(spec 決策)
git rm -rf sources
# Task 4 已 git mv 走 notes/entities/_templates 的內容;清掉殘留佔位 + 空目錄
...
- **Spec coverage:** A→T1/T3; B→T6; C(CLI)→T6/T5; D(install)→T6; E(template)→T4; F(skills)→T7; G(two CLAUDE.md)→T4(brain)+T8(engine); H(README/package)→T8; I(migration)→T8 README; J(lens HEAD)→T4 Step 3. `lib/config.mjs`/`where`/`use` impl→T1/T2. `bin/` fixups→T6. AC4 delete-repo→T8 test. PATH probe→T7+T3(doctor). All spec sections map to a task.
...
- **Ordering:** install (T6) follows template (T4) + init (T5), so `install.sh`'s `cp -R` of `init.mjs`/`_brain-template/` always has its inputs — every per-task test passes standalone. `_templates/lens.md` is explicitly removed in T8 Step 6, and T8 Step 8 asserts `_templates` is gone. `bin/test-brain-root.sh` (which `cd`s into `notes/`) is deleted in T6 before T8 removes `notes/`.
```
