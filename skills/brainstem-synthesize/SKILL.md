...
description: 從你的知識圖譜(brain)的 notes/entities 合成文章草稿,落到設定的 drafts 目錄(預設同腦的 docs/drafts/)。產帶 front-matter 與合成來源註解的草稿、補 log。Trigger:「產草稿」/「合成一篇」/「腦→草稿」。
---
...
3. **去重**:先掃 `$DRAFTS/` + 相關 `notes/`,確認此主題沒寫過;命中 → 停下問要不要更新,別重寫。
...
2. **寫草稿** → `$DRAFTS/<slug>.md`(寫前先 `mkdir -p "$DRAFTS"`)(slug kebab-case、英文意譯優先;沿用既有 note slug、不另造同義詞)。front-matter **欄位固定**:
   ```yaml
   ---
   title: "…"
...
4. **補 log**:`$BRAIN/log.md` append 一行 `YYYY-MM-DD synthesize — <來源> → $DRAFTS/<slug>.md(待 review)`。
5. **體檢**:草稿在 `$DRAFTS/`,**不進 wikilink 圖** → 不必跑 check.mjs;只有當你**同時動到** `notes/`/`entities/` 才跑 `brainstem check` 確認 0 孤島/0 斷鏈。
